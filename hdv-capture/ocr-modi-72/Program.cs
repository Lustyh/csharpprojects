...
namespace ocr_modi
{
    class Program
    {
        static void Main(string[] args)
        {
            ...
            {
                //Console.Error.WriteLine("ocr-modi.exe [path] [format_index] [xsize] [ysize]");
                Console.Error.WriteLine("ocr-modi.exe [path] [format_index]");
                return;
            }
            ...
            if (args.Length == 2)
            {
                try
                {
                    Capture.format = uint.Parse(args[1]);
                    //Capture.WIDTH = int.Parse(args[2]);
                    //Capture.HEIGHT = int.Parse(args[3]);
                }
                catch(Exception)
                {
                    Console.WriteLine("arg2 must be integer!");
                }
            }
            try
            {
                ...
                for (int i = 0; i < 5; i++)
                {
                    ...
                    try
                    {
                        Bitmap mini = Capture.ResizePicture(pic, new Size(960, 540));
                        mini.Save(filepath);
